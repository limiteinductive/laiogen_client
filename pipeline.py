...
def initialize_plasma(checkpoint_path="./stable-diffusion.pt") -> StableDiffusionPlasma:
    ...
    for field, model in model_dict.items():
        plasma[field] = (
            push_model_to_plasma(model) if isinstance(model, torch.nn.Module) else model
        )
...
def run_stable_diffusion(
    ...
    with load_from_plasma(plasma.safety_checker, device=device) as safety_checker:
        safety_checker_input = plasma.feature_extractor(
            pil_images, return_tensors="pt"
        ).to(device)
        output_images, has_nsfw_concept = safety_checker(
            clip_input=safety_checker_input.pixel_values, images=images
        )
